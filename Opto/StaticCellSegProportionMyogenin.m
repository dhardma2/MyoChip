...
prompt = {'Pixel width (microns):','Day:','Position:','Enhancement factor:','image length:','Dilation factor:','% Myogenin threshold:'};
dlgtitle = 'Input';
dims = [1 35];
...
Myogenin_Thresh=str2double(answer(7));
L2=round(L1/ps,0);
...
%Read Myogenin stained regions for observation
Myogenin=imread(filename1)*EF;
Myogenin=Myogenin(1:L2,1:L2);
...
%Read Myogenin stained regions
Myogenin_Nuclei=imread(filename4);
Myogenin_Nuclei=Myogenin_Nuclei(1:L2,1:L2);
Myogenin_Nuclei=imbinarize(Myogenin_Nuclei);

Myogenin_Nuclei=imdilate(Myogenin_Nuclei,seMB);
%Calculate overlapping regions of Myogenin and DAPI (Myogenin inside
%nuclei)
Myogenin_Nuclei=Myogenin_Nuclei & NucDil;
%figure;imshow(MBNuc);
...
%NucDil=Nuc;
%Search through nuclei regions and count the number of Myogenin positive
%pixels
...
for px1=1:length(StatsNucPx.PixelList)
    ...
    for px2=1:length(Pixels1)
        if logical(Myogenin_Nuclei(Pixels1(px2,2),Pixels1(px2,1)))
            test(Pixels1(px2,2),Pixels1(px2,1))=1;
            pxcount=pxcount+1;
        end
    end
    %Calculate proportion of cell with Myogenin staining
    Proportion_Myogenin(px1)=pxcount/length(Pixels1);
    pxcount=0;
end       
...
%Show overlay of DAPI and Myogenin
C=imfuse(Nuclei(1:L2,1:L2), Myogenin(1:L2,1:L2),'falsecolor','Scaling','joint','ColorChannels',[1 2 0]);  
figure(1);imshow(C);
...
for k=1:px1
    if Proportion_Myogenin(k)<=Myogenin_Thresh
...
[AddMBx,AddMBy,RemMBx,RemMBy]=ManLab(Myogenin);
if sum(AddMBx)>0
    AddMB=length(AddMBx);
else
    AddMB=0;
...
[RemMTx,RemMTy,AddMTx,AddMTy]=ManLab(Myogenin);
if sum(AddMTx)>0
    AddMT=length(AddMTx);
else
    AddMT=0;
...
clear C MBNuc Nuc NucBW NucDil Myogenin Nuclei Myogenin_Nuclei I Myotube BWskel test
